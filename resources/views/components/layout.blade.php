    ...
    <nav class="md:flex md:justify-between md:items-center">
        ...
        <div class="mt-8 md:mt-0 flex">
            @auth
                <x-dropdown>
                    <x-slot name="trigger">
                        <button class="flex mt-3 text-xs font-bold uppercase">Welcome, {{ auth()->user()->name }}</button>
                    </x-slot>
                    ...
                    <form id="logout-form" action="/logout" method="POST" class="flex ml-2 hidden">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-sm font-semibold uppercase text-red-500">Logout</button>
                    </form>
                </x-dropdown>
                <a href="#newsletter"
            ...
            @else
                <a href="/login" class="flex mt-3 text-xs font-bold uppercase">Login</a>
